...
const resType = resourceType.service;
...
export default function ServiceList() {
    ...
    const listParam = {
        title: "Service 服务",
        description: "将在集群中运行的Pod应用公开为可访问的网络服务。",
        filterArea: <Box
            block
            left={
                <Space>
...
const getCells = (t: TFunction) => {
    return [
        ...
        {head: '内部IP', paramName: 'ip'},
        {head: '外部IP', paramName: 'externalIp'},
        {head: '服务端口', paramName: 'port'},
...
const getData = (data: PageList, isSuccess: boolean) => {
    if (isSuccess) {
        return data.items.map(((item: Service, index) => {
            let externalIp= "-"
