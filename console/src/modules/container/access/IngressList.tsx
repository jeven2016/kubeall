...
const resType = resourceType.ingress;
...
export default function IngressList() {
    ...
    const listParam = {
        title: "Ingress 服务",
        description: "Ingress 是对集群中服务的外部访问进行管理的 API 对象，典型的访问方式是 HTTP。" +
...
const getCells = (t: TFunction) => {
    return [
        ...
        {head: 'Class', paramName: 'class'},
        {head: '地址', paramName: 'host'},
        {head: 'IP', paramName: 'ip'},
        {head: '端口', paramName: 'port'},
...
const getData = (data: PageList, isSuccess: boolean) => {
    if (isSuccess) {
        return data.items.map(((item: Ingress, index) => {
            let ip = "-"

            const ings = item.status?.loadBalancer?.ingress
            ...
            if (rules && rules.length > 0) {
                const hostArray: string[] = [];
                for (let h of rules) {
                    hostArray.push(h.host ?? "");
                }
                host = hostArray.join(", ");
            }
