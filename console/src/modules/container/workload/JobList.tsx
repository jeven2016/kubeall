...
const resType = resourceType.job;
...
export default function JobList() {
    ...
    const listParam = {
        title: "Job 任务",
        description: "Job 表示一次性任务，运行完成后就会停止。",
        filterArea: <Box
            block
            left={
                <Space>
...
const getData = (data: PageList, isSuccess: boolean) => {
    if (isSuccess) {
        return data.items.map(((item: Job, index) => {
            const replicas = item.spec?.completions ?? 0;
            const readyReplicas = item.status?.succeeded ?? 0;

            let status = <></>
            if (replicas === replicas && replicas > 0) {
                status = <Badge type="tag" color="ok">完成{` (${readyReplicas}/${replicas})`}</Badge>;
            ...
            const endTime = new Date(item.status!!.completionTime!!)
