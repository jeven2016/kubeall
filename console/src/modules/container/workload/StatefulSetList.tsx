...
const resType = resourceType.statefulSet;
...
export default function StatefulSetList() {
    ...
    const listParam = {
        ...
        description: "StatefulSet 运行一组 Pod，并为每个 Pod 保留一个稳定的标识。 这可用于管理需要持久化存储或稳定、唯一网络标识的应用。",
        filterArea: <Box
            block
            left={
                <Space>
...
const getData = (data: PageList, isSuccess: boolean) => {
    if (isSuccess) {
        return data.items.map(((item: StatefulSet, index) => {
