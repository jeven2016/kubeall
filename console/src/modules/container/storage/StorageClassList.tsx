...
export default function StorageClassList() {
    ...
    const listParam = {
        title: "StorageClass 存储类",
        description: "Kubernetes 存储类的概念类似于一些其他存储系统设计中的\"配置文件\",使管理员能够按需创建存储卷。",
        filterArea: <Box
            block
            left={
                <Space>
...
const getCells = (t: TFunction) => {
    return [
        ...
        {head: '存储驱动', paramName: 'provisioner'},
        {head: '回收策略', paramName: 'reclaimPolicy'},
        {head: '卷绑定模型', paramName: 'volumeBindingMode'},
...
const getData = (data: PageList, isSuccess: boolean) => {
    if (isSuccess) {
        return data.items.map(((item: StorageClass, index) => {
