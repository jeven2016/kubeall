...
const resType = resourceType.pvc;
...
export default function PvcList() {
    ...
    const listParam = {
        title: "Persistent Volume Claim 持久卷申明",
        description: "PV 卷是集群中的存储资源。PVC 申领是对这些资源的请求，也被用来执行对资源的申领检查。",
        filterArea: <Box
            block
            left={
                <Space>
...
const getCells = (t: TFunction) => {
    return [
        ...
        {head: '持久卷', paramName: 'pv'},
        {head: '容量', paramName: 'capacity'},
        {head: '访问模式', paramName: 'accessMode'},
        {head: '存储类', paramName: 'sc'},
...
const getData = (data: PageList, isSuccess: boolean) => {
    if (isSuccess) {
        return data.items.map(((item: PersistentVolumeClaim, index) => {
